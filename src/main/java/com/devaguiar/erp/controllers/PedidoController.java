...
public class PedidoController {
    ...
    @PostMapping
    public ResponseEntity<PedidoResponseDTO> createPedido(@RequestBody PedidoRequestDTO data) {
        PedidoResponseDTO created = pedidoService.createPedido(data);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }

    @PutMapping("/{id}")
    public ResponseEntity<PedidoResponseDTO> updatePedido(@PathVariable Long id, @RequestBody PedidoRequestDTO data) {
